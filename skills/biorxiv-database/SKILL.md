...
description: Source-specific bioRxiv search and PDF retrieval for life-science preprints. Use for bioRxiv-only filtering by category, author, or date range. Do not use this as a general literature review or writing skill.
license: Unknown
metadata:
...
This skill is the bioRxiv-specialized companion to the main workflow.

- Prefer `literature-search` for cross-source initial discovery
- Prefer `pubmed-database` when MeSH terms or clinical filtering matter
- Prefer `systematic-review` when you need a full phase-gated review workspace

## Do Not Use This Skill For

- general multi-source paper triage
- narrative literature synthesis
- manuscript writing or bibliography repair

## Interoperability

To reuse this skill's raw JSON inside the repository workflow, normalize it first:
...
Then merge the normalized JSONL into `outputs/<topic-slug>/paper_db.jsonl`.
