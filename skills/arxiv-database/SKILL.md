---
name: arxiv-database
description: Source-specific arXiv search and PDF retrieval. Use when you need arXiv-only queries, category browsing, ID lookup, or bulk preprint downloads. Do not use this as the default multi-source literature search entry point.
license: MIT
metadata:
...
This is a source-specific retrieval skill, not the repository's main discovery orchestrator.

- Prefer `literature-search` for initial multi-source paper collection
- Prefer `systematic-review` if you are building a full review workspace
- Use this skill when arXiv syntax, category filters, or PDF downloads matter

## Do Not Use This Skill For

- multi-source ranking and deduplication
- thematic literature synthesis
- bibliography validation or writing

## Interoperability

If you save raw output from this skill, normalize it before merging it into the shared corpus:
...
The normalized JSONL can then be merged into `outputs/<topic-slug>/paper_db.jsonl`.
