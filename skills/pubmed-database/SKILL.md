...
This skill is the biomedical search-strategy specialist.

- Use it when query correctness, MeSH expansion, or PubMed-specific filtering matters
- Prefer `literature-search` for fast general discovery across multiple sources
- Prefer `systematic-review` when PubMed retrieval is only one phase inside a larger review workflow

## Do Not Use This Skill For

- general cross-domain literature search
- bibliography-only cleanup
- writing review prose or a survey manuscript

## Interoperability

When PubMed search results are part of a larger repository workflow, preserve at least:
